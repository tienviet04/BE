...
const processMovies = async (movies) => {
    for (const movieData of movies) {
        ...
        const existingMovie = await NewMovies.findOne({ slug: movieDetail.slug });

        if (existingMovie) {
            // Cập nhật phim nếu đã tồn tại
            await NewMovies.findByIdAndUpdate(existingMovie._id, {
...
const connectAndFetchMovies = async () => {
    try {
        ...
        await fetchAndProcessMovies(1); // Thay đổi số trang cần lấy nếu cần
